...
export default function Intro({ detailss, visitor, setOthername }) {
  ...
  useEffect(() => {
    setDetails(initial);
    setInfos(initial);
    setMax(detailss&&100- detailss.bio.length)
  }, [detailss]);
  const initial = {
  ...
  const updateDetails = async () => {
    try {
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}/api/users/updateDetails`,
        {
          infos,
        },
        {
  ...
  const cancelDetails = async (name) => {
    setInfos((p)=>({...p, [name]: details?.[name]}))
  };
